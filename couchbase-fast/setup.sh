...
cbq -e http://localhost:8093 -u gs -p admin123 -s "CREATE PRIMARY INDEX ON \`data\`;"
...
couchbase-cli user-manage -c localhost -u gs -p admin123 --set --rbac-username data --rbac-password admin123 --roles admin --auth-domain local
